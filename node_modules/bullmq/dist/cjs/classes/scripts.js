...
class Scripts {
    ...
    static finishedErrors(code, jobId, command, state) {
        switch (code) {
            case enums_1.ErrorCodes.JobNotExist:
                return new Error(`Missing key for job ${jobId}. ${command}`);
            case enums_1.ErrorCodes.JobLockNotExist:
                return new Error(`Missing lock for job ${jobId}. ${command}`);
            case enums_1.ErrorCodes.JobNotInState:
                return new Error(`Job ${jobId} is not in the ${state} state. ${command}`);
            case enums_1.ErrorCodes.JobPendingDependencies:
                return new Error(`Job ${jobId} has pending dependencies. ${command}`);
            case enums_1.ErrorCodes.ParentJobNotExist:
